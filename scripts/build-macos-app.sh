...
for required_tool in dotnet plutil file; do
...
icns_source="$repo_root/src/GrayscaleLayersMac/Assets/AppIcon.icns"
...
[[ -s "$icns_source" ]] || { echo "ICNS 图标不存在：$icns_source" >&2; exit 1; }
...
mkdir -p "$macos_path" "$resources_path/scripts"
cp -R "$publish_path/." "$macos_path/"
cp "$plist_source" "$staged_app/Contents/Info.plist"
cp "$icns_source" "$resources_path/AppIcon.icns"

for script_name in grayscale_layers.py texture_to_hatch_dxf.py dxf_to_machine_file.py laser_pmt.py laser_timestamp.py; do
